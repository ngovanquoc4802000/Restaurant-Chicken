      ...
      <div className="register-container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 py-0 md:px-20 lg:px-20">
        ...
        <div className="register-form p-12 md:p-12 md:mt-3.5 flex-col justify-center">
          ...
          <form className="flex flex-col " onSubmit={handleSubmit}>
            <InputValue classNameLabel="md:text-[18px]" classNameInput={' p-2 border border-gray-500 rounded mt-1 w-full'} text="Họ và tên của bạn *" type="text" name="fullname" value={value.fullname} onChange={onChangeRegister} />
            <InputValue classNameLabel="md:text-[18px]" text="Số điện thoại" value={value.phone_number} type="tel" name="phone_number" onChange={onChangeRegister} classNameInput={'w-full p-2 border border-gray-500 rounded mt-1'} />
            <InputValue classNameLabel="md:text-[18px]" classNameInput={'w-full p-2 border border-gray-500 rounded mt-1'} type="email" name="email" value={value.email} text="Email của bạn *" onChange={onChangeRegister} />
            <InputValue classNameLabel="md:text-[18px]" text="Địa chỉ của bạn *" type="address" name="address" value={value.address} onChange={onChangeRegister} classNameInput={'w-full p-2 border border-gray-500 rounded mt-1'} />
            <InputValue classNameLabel="md:text-[18px]" text="Mật khẩu *" type="password" name="password" value={value.password} onChange={onChangeRegister} classNameInput="w-full p-2 border border-gray-500 rounded mt-1" />
            ...
            <Button className="btn-register mt-8 bg-[#e4002b] text-[16px] md:text-[18px] text-white p-3 border-none rounded-3xl font-bold cursor-pointer text-[1.1rem]" text=" Tạo Tài Khoản" type="submit" />
          </form>
          <div className="login text-center p-3">
            <label htmlFor="" className="text-[16px] md:text-[18px]">Bạn đã có tài khoản</label>
