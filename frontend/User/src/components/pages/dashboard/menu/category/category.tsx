...
function Categories({ category } : CategoriesTs) {
  return (
    <section className="menu-section p-5">
      ...
      <div className="menu-grid grid gap-5 py-0 px-3">
        {
          category.map((item) => (
            <NavLink style={{ color: "#000", textDecoration: "none", fontWeight: "700", fontSize: "1.2rem" }} className="menu-link" to="menu">
              <div className="menu-item bg-white rounded-lg overflow-hidden transition-transform duration-200 ease-in" key={item.id}>
