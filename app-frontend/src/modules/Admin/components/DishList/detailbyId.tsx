      ...
      <div className="modal-content bg-white p-5 rounded-[5px] w-[600px] max-w-[90%] shadow-2xl">
        ...
        {item && item.length > 0 ? (
          item.map(({ alt_text, image }, index) => (
            <div className="modal-details flex flex-col mb-4 border border-solid border-gray-300 rounded-[5px] p-2.5 bg-[#f9f9f9]" key={index}>
              {image ? (
                <img
        ...
        <button  className="mt-5 px-[10px] py-[15px] bg-[#007bff] text-white border-none rounded-[5px] cursor-pointer hover:bg-[#0056b3]" onClick={onHideModal}>Close</button>
      </div>
    </div>
  );
}
