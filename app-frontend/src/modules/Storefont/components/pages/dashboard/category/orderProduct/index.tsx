...
function orderProductDashBoard() {
  ...
  const findId = orderList?.filter((item) => item.user_id === userId);
  console.log(findId);
  /*  useEffect(() => {
    if(rule === "customer" && findId && userId) {
      console.log("đã lấy thành công" + findId + userId)
    }
  },[findId,rule,userId]) */
  const largerId = findId?.map((item) => item.details.length > 0);
    ...
    <div>
      ...
      {rule === "customer" && largerId ? (
        <div className="max-w-7xl mx-auto md:mt-[6rem] lg:mt-[0px] xl:mt-[0px] px-4 py-8 mt-16">
          ...
          {findId?.map((item) => item.details.map((details) => (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                >
                  <div className="md:flex lg:flex xl:flex">
                    ...
                    <div className="flex-1 ml-3">
                      ...
                      <p className="text-gray-600">Quantity: {item.details.map((item) => item.quantity)}</p>
                      <p className="text-gray-600">Price: {details.price} VND</p>
              ...
              <div className="bg-white  rounded-lg shadow-2xl min-h-full p-10 h-fit">
                <h2 className="text-xl font-bold mb-4">{quantity} dish</h2>
                <div className="mb-4">
