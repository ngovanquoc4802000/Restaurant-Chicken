...
function Categories({ category }: CategoriesTs) {
  return (
    <div className="xl:max-w-[1200px] xl:m-auto md:pt-[2rem] ">
      ...
      <section className="menu-section  gap-5 py-0 px-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 ">
        {category.map((item) => (
          <NavLink
            style={{ color: "#000", textDecoration: "none", fontWeight: "700", fontSize: "1.2rem" }}
            className="menu-link"
            to="menu"
          >
            <div
              className="menu-item hover:scale-105 shadow-md bg-white rounded-lg md:rounded-xl lg:rounded-2xl  overflow-hidden transition-transform duration-200 ease-in "
              key={item.id}
            >
